...
bef <- data.frame(
  ...
  FOED_DAG = runif(n, as.Date("1950-01-01"), as.Date("1989-12-31")) |> floor()  |> as.Date()
)
```
...
#lpr_diag |> group_by(RECNUM) |> mutate(ctype = paste(C_DIAGTYPE, collapse = "+")) |> reframe(n = n(), ctype = max(ctype)) |> summarise(n = n(), .by=ctype)
```
...
lab_dm_forsker <- data.frame(
  ...
  VALUE = rnorm(n_lab, mean = 70, sd = 10) |> round()
)
```
...
last_eGFR <- last_eGFR |>
  ...
  rename(last_eGFR = VALUE, eGFR_date = SAMPLINGDATE)
...
pop <- pop |>
  filter(is.na(EKSD) | D_INDDTO <= EKSD) |>
  select(-c(EKSD))
```
...
lmdb_metformin <- lmdb_metformin |>
  ...
  mutate(daily_doses = sum(APK * PACKSIZE),
         total_dose = sum(APK * PACKSIZE * STRNUM)) |>
  # keep one line per person
  slice(1) |>
  ungroup() |>
  select(c(PNR, EKSD, daily_doses, total_dose))
```
...
```{r}
pop <- pop |>
  mutate(tte_metformin = difftime(EKSD, D_INDDTO))
         
```

## Now you can start analysing the data...
```{r}
plot(pop$last_eGFR, pop$tte_metformin)
```
